...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///./ai_tele.db", description="Database connection URL"
    )
    ...
    ssh_key_path: Optional[str] = Field(
        default=None, description="SSH private key path"
    )
    ssh_remote_host: str = Field(
        default="localhost", description="Remote database host"
    )
    ...
    wechat_bot_url: str = Field(
        default="https://wxbot.da-mai.com/api/v1/wxbot/user/send",
        description="WeChat bot API URL",
    )
    wechat_bot_token: Optional[str] = Field(
        default="damai2025wxbot", description="WeChat bot authorization token"
    )
    wechat_default_wxid: Optional[str] = Field(
        default="58065692621@chatroom",
        description="Default WeChat ID for notifications",
    )
    ...
    worker_count: int = Field(
        default=4, description="Number of event processing workers"
    )
    max_queue_size: int = Field(
        default=1000, description="Maximum size of each event queue"
    )
    dead_letter_queue_size: int = Field(
        default=100, description="Maximum size of dead letter queue"
    )
    default_wait_for_result: bool = Field(
        default=True, description="Default wait for result"
    )
    default_timeout: float = Field(
        default=30.0, description="Default event timeout in seconds"
    )
    max_retry_count: int = Field(
        default=3, description="Maximum number of event retries"
    )
    ...
    enable_persistence: bool = Field(
        default=True, description="Enable event persistence to disk"
    )
    persistence_path: str = Field(
        default="./logs/events", description="Path for event persistence"
    )
