...
class ScheduledTasks(Base):
    ...
    interval_minutes: Mapped[Optional[int]] = mapped_column(Integer, comment="间隔分钟数")
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, comment="是否启用")
    description: Mapped[Optional[str]] = mapped_column(Text, comment="任务描述")
    last_run: Mapped[Optional[datetime]] = mapped_column(DateTime, comment="上次执行时间")
    next_run: Mapped[Optional[datetime]] = mapped_column(DateTime, comment="下次执行时间")
    ...
    # 关联关系
    execution_logs: Mapped[list["TaskExecutionLogs"]] = relationship("TaskExecutionLogs", back_populates="task")
...
class TaskExecutionLogs(Base):
    ...
    # 主键
    id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="主键ID")
    task_id: Mapped[int] = mapped_column(Integer, ForeignKey("scheduled_tasks.id"), comment="任务ID")
    advisor_id: Mapped[Optional[int]] = mapped_column(Integer, comment="顾问ID")
    execution_time: Mapped[Optional[datetime]] = mapped_column(DateTime, comment="执行时间")
    status: Mapped[str] = mapped_column(Enum("success", "failed", "running", name="task_status"), default="running", comment="执行状态")
    error_message: Mapped[Optional[str]] = mapped_column(Text, comment="错误信息")
    execution_duration: Mapped[Optional[int]] = mapped_column(Integer, comment="执行时长(秒)")

    # 时间字段
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now, comment="创建时间")

    # 关联关系
    task: Mapped["ScheduledTasks"] = relationship("ScheduledTasks", back_populates="execution_logs")