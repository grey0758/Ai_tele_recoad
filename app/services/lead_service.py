...
from app.schemas.lead import LeadCreate, LeadUpdate, LeadQueryParams, LeadListResponse, LeadResponse
...
class LeadService(BaseService):
    ...
    async def get_leads_by_advisor(self, advisor_id: int, limit: int = 10) -> Sequence[Lead]:
        """根据顾问ID获取线索列表"""
        async with self.database.get_session() as db_session:
            try:
                result = await db_session.execute(
    ...
    async def get_leads_by_category(self, category_id: int, limit: int = 10) -> Sequence[Lead]:
        """根据分类ID获取线索列表"""
        async with self.database.get_session() as db_session:
            try:
                result = await db_session.execute(
    ...
    # 私有辅助方法，避免重复代码
    async def _get_lead_by_id_with_session(self, db_session: AsyncSession, lead_id: int) -> Optional[Lead]:
        """在指定会话中根据ID获取线索"""
        result = await db_session.execute(
            select(Lead).where(Lead.id == lead_id)
        )
    ...
    async def _get_lead_by_lead_no_with_session(self, db_session: AsyncSession, lead_no: str) -> Optional[Lead]:
        """在指定会话中根据线索编号获取线索"""
        result = await db_session.execute(
            select(Lead).where(Lead.lead_no == lead_no)
        )
