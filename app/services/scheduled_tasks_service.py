...
class ScheduledTasksService(BaseService):
    ...
    async def get_all_scheduled_tasks(self) -> List[ScheduledTaskResponse]:
        ...
        async with self.database.get_session() as db_session:
            try:
                ...
                logger.info(f"成功获取 {len(task_responses)} 个定时任务")
    ...
    async def get_scheduled_task_by_id(self, task_id: int) -> Optional[ScheduledTaskResponse]:
        """根据ID获取定时任务"""
        async with self.database.get_session() as db_session:
            try:
    ...
    async def create_task_execution_log(self, log_data: TaskExecutionLogCreate) -> TaskExecutionLogResponse:
        """创建任务执行日志"""
        async with self.database.get_session() as db_session:
            try:
                # 验证任务是否存在
                task = await self._get_task_by_id_with_session(db_session, log_data.task_id)
                if not task:
                ...
                logger.info(f"成功创建任务执行日志: task_id={log_data.task_id}, status={log_data.status}")
                return log_response
                
            except Exception as e:
                await db_session.rollback()
                logger.error(f"创建任务执行日志失败: {e}")
                raise
    
    async def get_task_execution_logs_by_task_id(self, task_id: int, limit: int = 50) -> List[TaskExecutionLogResponse]:
        """根据任务ID获取执行日志列表"""
        async with self.database.get_session() as db_session:
            try:
                result = await db_session.execute(
                ...
                logger.info(f"成功获取任务 {task_id} 的 {len(log_responses)} 条执行日志")
    ...
    async def get_task_execution_logs_by_advisor_id(self, advisor_id: int, limit: int = 50) -> List[TaskExecutionLogResponse]:
        """根据顾问ID获取执行日志列表"""
        async with self.database.get_session() as db_session:
            try:
                result = await db_session.execute(
                ...
                logger.info(f"成功获取顾问 {advisor_id} 的 {len(log_responses)} 条执行日志")
                return log_responses
                
            except Exception as e:
                logger.error(f"获取顾问执行日志失败: {e}")
                raise
    
    async def get_all_task_execution_logs(self, limit: int = 100) -> List[TaskExecutionLogResponse]:
        """获取所有任务执行日志"""
        async with self.database.get_session() as db_session:
            try:
                result = await db_session.execute(
                ...
                logger.info(f"成功获取 {len(log_responses)} 条执行日志")
                return log_responses
                
            except Exception as e:
                logger.error(f"获取所有执行日志失败: {e}")
                raise
    
    # 私有辅助方法
    async def _get_task_by_id_with_session(self, db_session: AsyncSession, task_id: int) -> Optional[ScheduledTasks]:
        """在指定会话中根据ID获取定时任务"""
        result = await db_session.execute(
            select(ScheduledTasks).where(ScheduledTasks.id == task_id)
        )
