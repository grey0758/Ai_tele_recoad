...
async def get_leads(
    ...
    created_at_start: Optional[str] = Query(None, description="创建时间开始 (YYYY-MM-DD)"),
    created_at_end: Optional[str] = Query(None, description="创建时间结束 (YYYY-MM-DD)"),
    search: Optional[str] = Query(None, description="搜索关键词（客户姓名、电话、线索编号）"),
    ...
    try:
        ...
        if created_at_start:
            try:
                ...
                raise HTTPException(status_code=400, detail="创建时间开始格式错误，请使用 YYYY-MM-DD 格式")
        
        if created_at_end:
            try:
                created_at_end_dt = datetime.fromisoformat(created_at_end)
            except ValueError:
                raise HTTPException(status_code=400, detail="创建时间结束格式错误，请使用 YYYY-MM-DD 格式")
        ...
        raise HTTPException(status_code=500, detail=f"获取线索列表失败: {str(e)}")
...
async def get_lead_by_id(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取线索详情失败: {str(e)}")


@router.get("/leads/lead-no/{lead_no}", response_model=LeadResponse, summary="根据线索编号获取线索")
async def get_lead_by_lead_no(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"根据线索编号获取线索失败: {str(e)}")
...
async def update_lead(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"更新线索失败: {str(e)}")
...
async def delete_lead(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"删除线索失败: {str(e)}")


@router.get("/leads/advisor/{advisor_id}", response_model=List[LeadResponse], summary="根据顾问ID获取线索")
async def get_leads_by_advisor(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"根据顾问ID获取线索失败: {str(e)}")


@router.get("/leads/category/{category_id}", response_model=List[LeadResponse], summary="根据分类ID获取线索")
async def get_leads_by_category(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"根据分类ID获取线索失败: {str(e)}")


@router.get("/leads/status", response_model=List[LeadResponse], summary="根据状态获取线索")
async def get_leads_by_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"根据状态获取线索失败: {str(e)}")
