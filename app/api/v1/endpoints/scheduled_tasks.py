...
async def get_scheduled_task(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取定时任务详情失败: {str(e)}") from e
...
async def create_task_execution_log(
    ...
    try:
        ...
        return ResponseBuilder.success(data=log_response, message="创建任务执行日志成功")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"创建任务执行日志失败: {str(e)}") from e
