...
@router.post("/advisor-call-duration-stats", response_model=ResponseData[AdvisorCallDurationStatsResponse], summary="更新或插入顾问通话时长统计")
async def upsert_advisor_call_duration_stats(
    ...
    try:
        ...
        return ResponseBuilder.error(f"更新或插入顾问通话时长统计失败: {str(e)}", ResponseCode.INTERNAL_ERROR)


@router.get("/advisor-call-duration-stats/{advisor_id}/{stats_date}", response_model=ResponseData[AdvisorCallDurationStatsResponse], summary="获取顾问通话时长统计")
async def get_advisor_call_duration_stats(
    ...
    """
    try:
        stats = await aibox_service.get_advisor_call_duration_stats(advisor_id, stats_date)
        if not stats:
        ...
        return ResponseBuilder.error(f"获取顾问通话时长统计失败: {str(e)}", ResponseCode.INTERNAL_ERROR)


@router.get("/advisor-call-duration-stats/{advisor_id}/range", response_model=ResponseData[List[AdvisorCallDurationStatsResponse]], summary="获取顾问通话时长统计范围")
...
async def get_all_advisor_call_duration_stats_by_date(
    ...
    """
    根据指定日期获取所有顾问的通话时长统计数据
    """
    try:
        stats_list = await aibox_service.get_all_advisor_stats_by_date(stats_date)
        response_data = [AdvisorCallDurationStatsResponse.model_validate(stats) for stats in stats_list]
        return ResponseBuilder.success(response_data, f"获取{stats_date}所有顾问通话时长统计成功")
